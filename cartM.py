...
class ShoppingCart:
    ...
    #         cart_df.columns = ['id', 'NAME', 'PRICE', 'CATEGORY', 'SUBCATEGORY', 'quantity']
    #         df1 = pd.merge(shelf_df, cart_df, on='id', how='left')
    #         print(df1.columns)
    #
    #         with pd.option_context('display.max_rows', None, 'display.max_columns',
    #                                None):  # more options can be specified also
    #             print(df1)
    #         df1['quantity'] = df1['quantity'] - df1['quantity_y']
    #         df = df1[[col for col in df1.columns if ('cart' not in col) ]]\
    #             .drop(['quantity_x','quantity_y'], axis=1)
    #         self.shelf = df.to_dict('records')
    #         return self.shelf, self.name, self.cart_items
